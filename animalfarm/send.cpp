...
bool ipc_send_data_to_port(const char* data, DWORD dwToWrite) {
	...
	if (!PurgeComm(GlobalVar::g_hComm, PURGE_RXCLEAR)) {
		LOGMESSAGE("Failed to purge comm port.");
	}
	...
	if (!WriteFile(GlobalVar::g_hComm, data, dwToWrite, &dwWritten, &osWrite)) {
		if (GetLastError() != ERROR_IO_PENDING) {
			...
			if (!GetOverlappedResult(GlobalVar::g_hComm, &osWrite, &dwWritten, TRUE)) {
				...
				case WAIT_OBJECT_0:
					if (!GetOverlappedResult(GlobalVar::g_hComm, &osWrite, &dwWritten, FALSE))
						fRes = FALSE;
					else
